...
**Release commit:** `631ae6b`
**Package identity:** the ZIP is named `lunchbox-connect-631ae6b.zip`, and this
manifest and `docs/VERIFICATION_FINAL.md` both reference `631ae6b`. The manifest
and the regenerated report are committed as a thin packaging layer on top of
`631ae6b` (that is the commit whose code they describe); the archive contains
...
applies `supabase/migrations/0001`–`0032` verbatim, and runs 12 suites. Each
suite is mutation-tested (deliberately broken to prove it can fail). This pass
adds raw-path negatives to `verify_db_boundary` (tenant/role triggers, School
Admin cannot record or publish). Full report and the release decision:
...
- **`supabase/`** — `migrations/0001`–`0032` (schema, RLS, resolution/publish
  engine, meal library RPCs, class_staff, per-meal demand, analytics one-truth,
  the integrity pass 0029/0030/0031, and the tenant-integrity + permission
  correction 0032); `functions/admin-create-user/`; `config.toml`.
...
Migrations apply in numerical order, `0001` → `0032`. For production, follow
...
email-delivered self-activation; the structured
...
expected-vs-actual quantities; multi-kitchen routing; retention/deletion.
Deliveries, Ops and Absences remain honest `NOT_YET_DEFINED` shells. This pass
**removed** authorities earlier passes had invented in these areas rather than
inventing more. Nothing has been applied to the production database in this pass.
